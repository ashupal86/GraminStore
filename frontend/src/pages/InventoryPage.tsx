...
const InventoryPage = () => {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    if (!token) return;
    try {
      await apiService.post('/api/v1/inventory/items', newItem, token);
      setNewItem({
        name: '',
        description: '',
        category: '',
  ...
  const addFromCatalog = async (prod: any) => {
    if (!token) return;
    const name = prod.product_name || prod.generic_name || prod.brands || 'Unnamed Item';
    const sku = prod.code || prod._id || undefined;
    const qtyStr = prompt(`Enter quantity to add for ${name}:`, '1');
    ...
    try {
      ...
      console.error('Error adding from catalog:', e);
    }
  };
